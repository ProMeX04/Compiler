...
export function TestPanel({
  ...
  const Row = ({ index, style }: { index: number; style: React.CSSProperties }) => (
    <div style={style} className={`mb-4 p-3 border rounded transition-colors ${
      testCases[index].passed === undefined
        ? theme === "light"
          ? "border-gray-300 bg-gray-50"
          : "border-zinc-700 bg-zinc-900"
        : testCases[index].passed
        ? theme === "light"
      ...
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-2">
          ...
          {testCases[index].passed !== undefined && (
            <span
              className={`text-xs ${
                testCases[index].passed ? "text-green-500" : "text-red-500"
              }`}
            >
              {testCases[index].passed ? "✓ Passed" : "✗ Failed"}
            </span>
          )}
        </div>
        <button
      ...
      <div className="space-y-2">
        ...
        {testCases[index].actualOutput !== undefined && (
          <div>
            <label className="block text-xs mb-1 opacity-70">
              Actual Output:
            </label>
            <textarea
              value={testCases[index].actualOutput}
              readOnly
